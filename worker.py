...
from api_instance import API_custom as API
...
class Worker:
    ...
    # Each Worker is passed different details and given a corresponding number/uuid
    __number = None
    ...
    def __get_followers(self, followed_user_id):
        ...
        cursor = tweepy.Cursor(self.__api.get_api().get_followers, user_id = followed_user_id, count = 500).pages()
        # Iterate over pages of followers
        while True:
            try:
                # attempt to get next page of followers, then sleep.
                user_page = cursor.next()
                print("\t└ new page, sleeping ...")
                sleep(45)
                ...
                for user in user_page:
                    ...
                    #----------------------------------------#
                    #---------------------------------------------------------------#
                    # A WORKER SHOULD CHECK THE STATE OF SOME OUTSIDE VARIABLE, LIKE  #
    ...
    def __set_initial_poi(self) -> tweepy.User:
        ...
        init_user_at = self.__api.get_init_poi()
        poi = self.__api.get_api().get_user(screen_name = init_user_at)
        """
    ...
    def scrape_user(self) -> None:
        ...
        while True:
            ...
            poi_tup = get_next_poi(poi, self.__api.get_api())
            ...
            poi = self.__api.get_api().get_user(user_id = new_poi_id)
